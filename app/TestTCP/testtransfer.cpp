...
public:
    TestTransfer_Accept(Host *host, const std::unordered_map <std::string, std::string> &env) : SystemCallApplication(
            host), TCPApplication(this) {
        this->env = env;
    }
...
//						EXPECT_EQ(send_buffer[j], recv_buffer[j]);
                    ...
                    EXPECT_EQ(send_buffer[0], recv_buffer[0]);
...
public:
    TestTransfer_Connect(Host *host, const std::unordered_map <std::string, std::string> &env) : SystemCallApplication(
            host), TCPApplication(this) {
        this->env = env;
    }
...
char str_buffer[128];
snprintf(str_buffer,
sizeof(str_buffer), "%u.%u.%u.%u", ip1[0], ip1[1], ip1[2], ip1[3]);
std::string host1_ip(str_buffer);
snprintf(str_buffer,
sizeof(str_buffer), "%u.%u.%u.%u", ip2[0], ip2[1], ip2[2], ip2[3]);
...
TimeUtil::printTime(TimeUtil::makeTime(1000, TimeUtil::USEC), TimeUtil::USEC
);
...
TimeUtil::printTime(TimeUtil::makeTime(2000, TimeUtil::USEC), TimeUtil::USEC
);
connect_env["START_TIME"] =
TimeUtil::printTime(TimeUtil::makeTime(1, TimeUtil::SEC), TimeUtil::USEC
);

connect_env["CONNECT_ADDR"] =
host2_ip;
